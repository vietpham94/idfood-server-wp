...
<div id="product-<?php the_ID(); ?>" <?php wc_product_class($classes, $product); ?>>
    ...
    $cac_nha_cung_cap = get_field('cac_nha_cung_cap', get_the_ID());
    $cac_nha_cung_cap_khu_vuc = array();
    foreach ($cac_nha_cung_cap as $item) {
        $customer = new WC_Customer($item['nha_cung_cap']);
        if ($customerCity == strtoupper(vn_to_str($customer->get_city()))) {
            $cac_nha_cung_cap_khu_vuc[] = $customer;
            $cityVn = $customer->get_city();
        }
    }
    ?>
